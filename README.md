...
#### 2. 同一台机器激活不同许可证
- **解绑**同项目下该机器的其他许可证（`bound = 0`）
- 如果是 trial 类型，则**撤销**同机器的其他许可证（`revoked = 1`）
...
| 场景 | 处理方式 |
|------|----------|
| 首次请求 `/api/license/trial` | 创建并激活 trial 许可证（30分钟） |
| 再次请求 `/api/license/trial` | 重新激活 trial（保留原过期时间，不延长） |
| 同一机器有 trial 时激活其他许可证 | 撤销同机器的其他许可证 |
| trial 被管理员撤销后再次请求 | 返回"授权码已被撤销"错误 |

### 撤销 vs 解绑

| 操作 | SQL | 效果 |
|------|-----|------|
| **撤销 (revoke)** | `SET revoked = 1` | 许可证被禁用，保留绑定信息 |
| **解绑 (unbind)** | `SET machine_code = NULL, bound = 0` | 清除绑定信息，许可证可重新绑定 |
